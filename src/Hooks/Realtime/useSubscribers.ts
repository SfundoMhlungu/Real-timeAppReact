...
const useSubscribers = ({websocketUrl}: {websocketUrl: string}) => {
  const [webWorker, setWW] = useState(new Worker(new URL("../../Threads/Thread.js", import.meta.url)))
  const [localQueue, setQueu] = useState([])
   useEffect(()=> {
      if(webWorker){
         // console.log(webWorker)
         if(!socketStarted){
         console.log(webWorker)
         webWorker.postMessage({event: "Start_WS", data: {config: {url: websocketUrl}}})
